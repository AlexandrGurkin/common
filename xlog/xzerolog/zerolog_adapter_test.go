...
func BenchmarkXLog(b *testing.B) {
	log, _ := NewXZerolog(xlog.LoggerCfg{Level: "trace", Out: &mocks.BlackHoleStream{}})
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		log.Trace("ep")
	}
...
func TestNewXZerologSuccess(t *testing.T) {
	...
	log, _ := NewXZerolog(xlog.LoggerCfg{Level: "trace", Out: mockWriter})
	log.Trace("ep")
...
func Test_x_WithXField(t *testing.T) {
	...
	log1, _ := NewXZerolog(xlog.LoggerCfg{Level: "trace", Out: mockWriter})
	mockWriter.EXPECT().
